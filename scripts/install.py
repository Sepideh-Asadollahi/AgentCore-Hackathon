#!/usr/bin/env python3
"""Change Society hackathon installer (Python-first, idempotent)."""
...
DEMO_ENV = """# Generated by hackathon/scripts/install.py (demo profile). Safe for local judging.
# Full template with Model & Qwen API sections: hackathon/.env.example
...
def repo_root() -> Path:
    ...
    monorepo = script.parents[2]
    if (monorepo / "hackathon" / "scripts" / "install.py").is_file():
        return monorepo
    # Standalone AgentCore-Hackathon publish: scripts/install.py at repository root
    return script.parents[1]


def hackathon_root() -> Path:
    root = repo_root()
    nested = root / "hackathon"
    if nested.is_dir() and (nested / "scripts" / "install.py").is_file():
        return nested
    return root
...
def run_verify(root: Path, dry_run: bool) -> None:
    script = root / "hackathon" / "scripts" / "run-real-test.sh"
    if dry_run:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install Change Society hackathon demo dependencies.",
...
""",
    ...
    parser.add_argument(
        "--with-postgres",
        action="store_true",
        help="If Docker is available: start hackathon/deployments/compose.dev-postgres.yaml",
    )
    parser.add_argument(
        "--non-interactive",
        action="store_true",
    ...
    parser.add_argument("--skip-frontend", action="store_true", help="Do not run npm in hackathon/frontend.")
...
def apply_runtime(
    ...
    *,
    ...
    if runtime == "systemd":
        ...
        install_systemd_user_units(root, ht, dry_run=dry_run)
        return
    if runtime == "docker":
        start_docker_compose_stack(root, dry_run=dry_run)
        return
    print(
        "Runtime=manual: start API + UI yourself when ready.\n"
        "  Example API:  set -a && source hackathon/.env && set +a && "
...
def main() -> int:
    ...
    root = repo_root()
    ht = hackathon_root()
    requirements = ht / "requirements.txt"
    if not requirements.is_file():
        requirements = ht / "backend" / "change-society-service" / "requirements.txt"
    ...
    dev_requirements = ht / "requirements-dev.txt"
    ...
    if with_postgres:
        start_docker_postgres(root, dry_run=args.dry_run)
    ...
    print(
        ...
        "See hackathon/docs/07-deployment-and-operations.md for production compose."
    )
    return 0
